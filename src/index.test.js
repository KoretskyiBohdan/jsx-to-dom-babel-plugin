...
describe("jsx-dom", () => {
  ...
  it("should create div with attributes", () => {
    expect(<div data-value="12" tabIndex="1" title="test-div" />).toEqual(
      createDOMFromString(
  ...
  it("should work with falsy values", () => {
    expect(<div>{null}</div>).toEqual(createDOMFromString("<div></div>"));
    expect(<div>{false}</div>).toEqual(createDOMFromString("<div>false</div>"));
    expect(<div>{undefined}</div>).toEqual(
      createDOMFromString("<div>undefined</div>")
    );
    expect(<div>{0}</div>).toEqual(createDOMFromString("<div>0</div>"));
  });
